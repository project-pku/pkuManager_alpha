...
/// <summary>
/// Represents a collection of compiled dexes.
/// </summary>
public abstract class DexManager
{
    ...
    */
    ...
    public DexManager() : this(DataSourceType.Latest, null, null) { }
    ...
    /// <inheritdoc cref="ReadLatestDex(string)" path="/exception"/>
    public DexManager(string hash) : this(DataSourceType.Previous, hash, null) { }
    ...
    public DexManager(DirectoryInfo directory) : this(DataSourceType.Local, null, directory) { }

    //Constructs a ddm, and starts downloading the datadexes in the background.
    private DexManager(DataSourceType dataSource, string? hash = null, DirectoryInfo? dir = null)
    {
     ...
     * ------------------------------------
    */
    //Base URL of the dex repo
    protected abstract string ONLINE_BASE_URL { get; }
    ...
    private async Task<JsonDocument> ReadLatestDex(string name)
